<script>
  import { originPoint } from '$lib/stores/index.js';
  import { searchPlace, searchPOI } from '$lib/utils/geocode.js';
  import { trackPoints } from '$lib/stores/index.js';
  import { findNearestTrackPointIndex, haversine } from '$lib/utils/geo.js';
...
<div class="p-4 pb-20">
  <div class="flex items-center justify-between mb-4">
    <h2 class="text-xl font-bold text-gray-900 dark:text-white">Origin</h2>
    <button
  ...
  {#if $originPoint}
    <div class="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg">
      <p class="text-sm font-medium text-green-800 dark:text-green-200">Origin Set</p>
      <p class="text-xs text-green-600 dark:text-green-400 mt-1">
        {$originPoint.name || `Lat: ${$originPoint.lat.toFixed(6)}, Lon: ${$originPoint.lon.toFixed(6)}`}
      </p>
    </div>
  {/if}
</div>